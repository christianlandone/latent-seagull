...
#define MSBC_SAMPLES_PER_FRAME    20
...
uint8_t i2sContMgtBuffer[2*I2S_BLOCK_OVERHEAD_IN_BYTES * I2SCC26XX_QUEUE_SIZE] = {0};
...
static void processPCMStream( int16_t* buffer, int length);
    ...
    while (1)
    {
        ...
        if (events & SBP_I2S_FRAME_EVENT)
        {
          ...
          if (i2sStreamInProgress) {
            ...
            {
                // Flush on UART
                int16_t* pbi = (int16_t*)bufferRequest.bufferIn;

                for(idx = 0; idx < streamVariables.samplesPerFrame; idx++ )
                {
                    uartSamples[idx] = pbi[2*idx+1];
                }

                if (bufferRequest.bufferOut)
                {
                  memcpy(bufferRequest.bufferOut, bufferRequest.bufferIn, 2*streamVariables.samplesPerFrame * sizeof(int16_t));
                ...
                UART_write(uartHandle, (uint8_t*)uartSamples , MSBC_SAMPLES_PER_FRAME* sizeof(int16_t));
...
static void initCodecADC()
{
    ...
    val = PDN_HPB_10 | PDN_HPA_10 | PDN_LINB_10 | PDN_LINA_10;
    SensorI2C_writeReg(PowerCTL2, &val, 1);

    val = 0x75;
    SensorI2C_writeReg(0x1A, &val, 1);
...
static void initI2SBus_In()
{
    ...
    pcmSamples = Memory_alloc(NULL, sizeof(int16_t) * (4*streamVariables.samplesPerFrame * I2SCC26XX_QUEUE_SIZE), 0, NULL);
    ...
    i2sParams.ui32conBufTotalSize    = sizeof(int16_t) * (4*streamVariables.samplesPerFrame * I2SCC26XX_QUEUE_SIZE);
