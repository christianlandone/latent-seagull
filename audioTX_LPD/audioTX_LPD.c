...
#define MSBC_SAMPLES_PER_FRAME    96
...
struct {
  ...
  int16_t pv;
  uint8_t activeLED;
} streamVariables = {STREAM_STATE_IDLE, STREAM_STATE_IDLE, 0, 0, 0, 0, 0, 0};
        ...
        if (events & SBP_I2S_FRAME_EVENT)
        {
            ...
            if (i2sStreamInProgress)
            {
                ...
                while (gotBuffer)
                {
                    ...
                    RF_cmdPropTxAdv.pPkt = (uint8_t*)packedSamples;//packedSamples;
